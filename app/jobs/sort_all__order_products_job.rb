class SortAllOrderProductsJob < ApplicationJob
  queue_as :default

  def perform(load_id)
    load = Load.find_by(id: load_id)
    if load.nil?
      Rails.logger.info("Load não encontrada ID: #{load_id}")
      return
    end
    ...
    Rails.logger.info("Todas as listas foram ordenadas na Load ID: #{load_id}")
  end
